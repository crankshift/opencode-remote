...
export function createTelegramSkillsMenu({
  ...
  return {
    async handleCommand(ctx) {
      ...
      const bundledMemeStatus = await bundledMemeRuntimeStatus()
      logger?.debug?.(
      ...
      if (state.step === "confirm") {
        ...
        await reply(
          ...
          [
            ...
            `Path: .opencode/skills/opencode-remote-generated/${created.skillName}/SKILL.md`,
...
function formatSkillsList({ skills = [], remoteSkillUrls = [] } = {}, { bundledMemeStatus } = {}) {
  ...
  sections.push(formatBundledMemeStatus(bundledMemeStatus))
...
function formatBundledMemeStatus(status) {

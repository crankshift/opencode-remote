...
description: Use when a user asks OpenCode Remote to create, generate, draft, or improve a user or project OpenCode skill.
license: MIT
...
- Use a global generated skill only when the user explicitly asks for a user-wide skill.
- Generated skills are user-owned OpenCode configuration, not OpenCode Remote runtime protocol code.
...
- Do not put credentials, API keys, Telegram credentials, numeric chat or user identifiers, private file locations, logs, message text, or private config values into generated skills.

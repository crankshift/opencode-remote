...
description: Use when working on Telegram sticker understanding, saved sticker packs, sticker catalogs, animated sticker previews, or hidden telegram_sticker markers.
license: MIT
...
- Do not invent Telegram file IDs, raw pack internals, or download URLs.

## Privacy Rules

- Do not expose bot tokens, chat IDs, user IDs, Telegram file IDs, raw download URLs, local cache paths, or raw Telegram payloads.
...
- Telegram sticker download, rendering, cache, saved-pack state, and marker parsing belong in `src/adapters/telegram/`.
...
- Update focused sticker tests under `tests/adapters/`.

...
`/skills` can also enable bundled OpenCode Remote runtime assets for the current project. Bundled assets are installed project-locally so OpenCode can discover them from `.opencode/`; they do not write global OpenCode config by default. The bundled meme workflow writes `.opencode/skills/opencode-remote-bundled/meme-generation/SKILL.md`. Enabling it also removes the old experimental `.opencode/agent/opencode-remote-meme.md` agent if present. Restart OpenCode if a newly enabled skill is not discovered immediately.

When the bundled meme skill is enabled, OpenCode can use it from the active session. The gateway does not force meme requests to a specific workflow; it only installs discoverable skill guidance and delivers validated `MEDIA:` output. Imgflip is used for meme template discovery and template image fetching only. Generated memes are rendered locally into the gateway generated-media cache and sent back through Telegram.

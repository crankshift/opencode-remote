...
`/skills` lists local skills the gateway can discover from the current OpenCode project, including default `.opencode/skills`, configured `skills.paths` in OpenCode config, compatible `.claude/skills` and `.agents/skills` folders, and generated skills created by OpenCode Remote. Remote `skills.urls` are reported but not listed by the gateway yet. `/skills create`, the `New skill` menu button, or a private chat request such as "please create a skill" starts a guided flow that writes a project-local generated skill under `.opencode/skills/opencode-remote-generated/<skill-name>/SKILL.md` after preview and confirmation. Generated skills are user-owned project configuration; avoid putting secrets, numeric Telegram identifiers, private file locations, logs, API keys, or private config values in them.

Gateway startup installs bundled OpenCode Remote runtime assets for the current project. If the project OpenCode config has a project-local `skills.paths` entry, bundled assets are installed under the first such path as `opencode-remote-bundled/<skill-name>/SKILL.md`; otherwise they are installed under `.opencode/skills/opencode-remote-bundled/<skill-name>/SKILL.md`. They do not write global OpenCode config by default. Startup and `/skills` refresh also remove the old experimental `.opencode/agent/opencode-remote-meme.md` agent if present. If OpenCode was already running before the gateway and does not discover a newly installed skill immediately, restart OpenCode.

When the bundled meme skill is installed, OpenCode can use it from the active session. The gateway does not force meme requests to a specific workflow; it only installs discoverable skill guidance and delivers validated `MEDIA:` output. Imgflip is used for meme template discovery and template image fetching only. Generated memes are rendered locally into the gateway generated-media cache and sent back through Telegram.
